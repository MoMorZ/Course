...
extern void FuncSetInt21();
extern void FuncRunInt21_1();
extern void FuncRunInt21_2();
extern void FuncRunInt21_3();
extern void FuncRunInt21_4();
extern void FuncGetDate();
extern void FuncGetTime();
extern void FuncAnother_load();
...
const char intTips1[] = "Please input number to choose the INT21 function";
...
const char mainTips8[] = "5.Call Int21";
const char mainTips9[] = "6.Parallel 4 Program";
...
void welcome() {
	...
	FuncPrintSL(mainTips9, strlen(mainTips9));
...
const char pProgTips1[] = "Tring to load progress...";
const char pProgTips2[] = "Load progress successfully!";

void prunProg(int Segment, int SecNo) {
	FuncPrintSL(pProgTips1, strlen(pProgTips1));
	FuncPrintSL("SecNo:", 6);
	FuncPrintC(SecNo + 48);
	FuncPrintSL("", 0);
	//FuncRunP(SecNo);
	FuncAnother_load(Segment, SecNo);
	FuncPrintSL(pProgTips2, strlen(pProgTips2));
	return;
}


void init_Pro() {
	init(&pcb_list[0], 0, 0xa100);
	init(&pcb_list[1], 0, 0x1000);
	init(&pcb_list[2], 0, 0x2000);
	init(&pcb_list[3], 0, 0x3000);
	init(&pcb_list[4], 0, 0x4000);
	init(&pcb_list[5], 0, 0x5000);
}


/*
void init_Pro() {
	init(&pcb_list[0], 0x0, 0xa100);
	init(&pcb_list[1], 0x1000, 0xc100);
	init(&pcb_list[2], 0x2000, 0xc100);
	init(&pcb_list[3], 0x3000, 0xc100);
	init(&pcb_list[4], 0x4000, 0xc100);
	init(&pcb_list[5], 0x5000, 0xc100);
}
*/

/*
void init_Pro() {
	init(&pcb_list[0], 0x1000, 0xa100);
...
void mycmain() {
	...
	while (1) {
		...
		case '6':
			//FuncRunP(6);
			FuncAnother_load(0x1000, 3);
			FuncAnother_load(0x2000, 4);
			FuncAnother_load(0x3000, 5);
			FuncAnother_load(0x4000, 6);
			Program_Num = 4;
			FuncSetClock();
			FuncClear();
			break;
